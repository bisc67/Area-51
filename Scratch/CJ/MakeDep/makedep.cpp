...
s32 FindIncludeNode( const xstring& IncludeFileName )
{
    ...
    for( s32 i=0 ; i<g_Nodes.GetCount() ; i++ )
    {
        node& n = g_Nodes[i];
...
void AddIncludeChain( s32 Index, s32 Target )
{
    ...
    for( s32 i=0 ; i<g_Nodes[Target].m_Includes.GetCount() ; i++ )
    {
        if( g_Nodes[Target].m_Includes[i] == Index )
            return;
    }
    g_Nodes[Target].m_Includes.Append( Index );

    // Append everything this depends on
    for( i=0 ; i<g_Nodes[Index].m_Includes.GetCount() ; i++ )
        AddIncludeChain( g_Nodes[Index].m_Includes[i], Target );
}

//=============================================================================

s32 ProcessFullyQualifiedFile( const xstring& FileName, const xstring& Key );
...
void ProcessInclude( const xstring& IncludeFileName, s32 Target )
{
    ...
    for( s32 i=0 ; i<g_IncludePaths.GetCount() ; i++ )
    {
        ...
        // Process it
        s32 Index = ProcessFullyQualifiedFile( Path, IncludeFileName );
        if( Index != -1 )
        {
...
s32 ProcessFullyQualifiedFile( const xstring& FileName, const xstring& Key )
{
    ...
    if( f.LoadFile( FileName ) )
    {
        ...
        s32     Index   = g_Nodes.GetCount();
        node&   n       = g_Nodes.Append();
...
void AddIncludePath( const xstring& Path )
{
    if( Path.GetLength() == 0 )
        return;
...
//            x_printf( "%s\n", g_SourceFileName );
            ...
            MD5Context md5;
            MD5Init( &md5 );
            MD5Update( &md5, g_Nodes[Index].m_MD5Digest, 16 );
            for( s32 j=0 ; j<g_Nodes[Index].m_Includes.GetCount() ; j++ )
            {
                MD5Update( &md5, g_Nodes[g_Nodes[Index].m_Includes[j]].m_MD5Digest, 16 );
            }

            {
                for( s32 k=0 ; k<16 ; k++ )
                {
                    x_printf( "%02X", g_Nodes[Index].m_MD5Digest[k] );
                }
                x_printf( "  %s\n", g_SourceFileName );
#if 0
                for( s32 j=0 ; j<g_Nodes[Index].m_Includes.GetCount() ; j++ )
                {
                    ...
                    for( s32 k=0 ; k<16 ; k++ )
                    {
                        x_printf( "%02X", g_Nodes[g_Nodes[Index].m_Includes[j]].m_MD5Digest[k] );
                    }
                    x_printf( "  %s\n", g_Nodes[g_Nodes[Index].m_Includes[j]].m_PathName );
                }
#endif
            }
        }
    ...
    x_printf( "Stats: Source = %d, Cached = %d, Opened = %d, Nodes = %d\n", CmdLine.GetNumArguments(), g_CacheHits, g_FilesOpened, g_Nodes.GetCount() );
