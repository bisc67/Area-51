...
public:
    ...
    char m_PersonalizeDependency[256];
    my_rsc_desc( void );
};
...
//
DEFINE_RSC_TYPE( s_Desc, my_rsc_desc, "xbmp", "tga", "This is a output bitmap resource" );

//DEFINE_RSC_TYPE( s_Desc, my_rsc_desc, "tga", "tga", "This is a input bitmap resource" );
...
{
    List.Append().Set( m_PersonalizeDependency, 0 );
}
...
{
    CompilerRules = xfs( "CompilerName.exe -INPUT test1.txt -PC c:\\%s.%s", GetName(), m_Type.GetName() );
}
...
void main ( void )
{
    ...
    //
    my_rsc_desc& Rsc1 = *((my_rsc_desc*)&s_DescMgr.CreateRscDesc( "HUD_ammo.xbmp" ));
    my_rsc_desc& Rsc2 = *((my_rsc_desc*)&s_DescMgr.CreateRscDesc( "HUD_grenade.xbmp" ));
    my_rsc_desc& Rsc3 = *((my_rsc_desc*)&s_DescMgr.CreateRscDesc( "HUD_health.xbmp" ));
    my_rsc_desc& Rsc4 = *((my_rsc_desc*)&s_DescMgr.CreateRscDesc( "HUD_recticle.xbmp" ));

//    List.Append().Set( "DependTest1.txt", 0 );

    x_strcpy( Rsc1.m_PersonalizeDependency, "HUD_ammo.tga" );
    x_strcpy( Rsc2.m_PersonalizeDependency, "HUD_grenade.tga" );
    x_strcpy( Rsc3.m_PersonalizeDependency, "HUD_health.tga" );
    x_strcpy( Rsc4.m_PersonalizeDependency, "HUD_recticle.tga" );
    ...
    //
    {
        ...
        {
            x_printf( "rscDesc: %s\n", s_DescMgr.GetRscDescIndex(i).pDesc->GetName() );
        }
    }
    ...
    while( RscIndex != -1 )
    {
//        x_printf( "rscDesc: %s\n", s_DescMgr.GetRscDescIndex(RscIndex).pDesc->GetName() );

        {
            ...
            // Spawn process
            hProcess = (HANDLE)spawnvpe(P_NOWAIT, Process, (const char* const*)Prameter, (const char* const*)Prameter);
            ...
            if(hProcess)
            {
                ...
                while   (nExitCode == STILL_ACTIVE)
                {
                    ...
                    if( nOutRead > 0 )
                    {
                        ...
                        for( s32 i=0; i<nOutRead; i++ )
                        {
                            x_printf("%c", szBuffer[i] );
                        }
