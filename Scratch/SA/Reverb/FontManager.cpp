...
void FontManager::Render ( void )
{
    ...
    for ( s32 i = 0; i < m_ShortStringCount; i++ )
    {
        ...
        {
            if( m_ShortString[i].m_Flags & USE_EMBEDDED_COLOR )
            {
    ...
    for ( s32 i = 0; i < m_MedStringCount; i++ )
    {
        ...
        {
            if( m_MedString[i].m_Flags & USE_EMBEDDED_COLOR )
            {
    ...
    for ( s32 i = 0; i < m_LongStringCount; i++ )
    {
        ...
        {
            if( m_LongString[i].m_Flags & USE_EMBEDDED_COLOR )
            {
...
void FontManager::RenderText ( const irect& R, u32 Flags, const xcolor& Color, const char* pString, s8 FontSize)
{
    ...
    if( StrLen <= SHORT_STRING )
    {
        ...
        // Copy from a narrow string to a wide string.
        for( StrLen=0; (m_ShortString[ m_ShortStringCount ].m_String[StrLen] = pString[StrLen]); StrLen++ );
    ...
    else if( StrLen <= MED_STRING )
    {
        ...
        // Copy from a narrow string to a wide string.
        for( StrLen=0; (m_MedString[ m_MedStringCount ].m_String[StrLen] = pString[StrLen]); StrLen++ );
    ...
    else if( StrLen <= LONG_STRING )
    {
        ...
        // Copy from a narrow string to a wide string.
        for( StrLen=0; (m_LongString[ m_LongStringCount ].m_String[StrLen] = pString[StrLen]); StrLen++ );
...
void FontManager::RenderText ( const irect& R, u32 Flags, s32 Alpha, const char* pString, s8 FontSize)
{
    ASSERT( pString );
...
void FontManager::RenderText ( const irect& R, u32 Flags, const xcolor& Color, const xwchar* pString, s8 FontSize)
{
    // Get the size of the string so we can put it in the correct string structure.
    s32 StrLen = 0;
    while( pString[StrLen] )
        StrLen++;
