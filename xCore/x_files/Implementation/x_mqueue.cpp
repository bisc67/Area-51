...
xarray<xmesgq*>     xmesgq::m_MasterMessageList(s_MesgqBuffer,0,X_MAX_MESSAGE_QUEUES);
#endif
...
xbool xmesgq::Send(void *message,s32 flags)
{
    ...
    else
    {
        ...
        while (IsFull())
        {
            ...
            if( (pThread->IsActive() == FALSE) && IsFull() )
            {
                if( pThread->m_pOwningQueue == &m_WaitingForRecv )
                {
                    pThread->Unlink( m_WaitingForRecv );
                }
                else
                {
    ...
    pThread = m_WaitingForSend.GetHead();
    if( pThread )
    {
        pThread->Unlink( m_WaitingForSend );
        x_EndAtomic();
...
void *xmesgq::Recv(s32 flags)
{
    ...
    else
    {
        ...
        while (IsEmpty())
        {
            ...
            if( (pThread->IsActive() == FALSE) && IsEmpty() )
            {
                if( pThread->m_pOwningQueue == &m_WaitingForSend )
                {
                    pThread->Unlink( m_WaitingForSend );
                }
                else
    ...
    // Do we have anything waiting on a send to complete?
    pThread = m_WaitingForRecv.GetHead();
    if( pThread )
    {
        pThread->Unlink( m_WaitingForRecv );
        x_EndAtomic();
