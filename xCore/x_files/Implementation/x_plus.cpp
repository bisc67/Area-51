...
//==============================================================================
//  "Mixed" string copy.  Copies wide string into narrow buffer.
...
//==============================================================================
//  "Mixed" string copy.  Copies narrow string into wide buffer.
...
//==============================================================================
//  "Mixed" string copy.  Copies wide string into narrow buffer. Max of Count
...
//==============================================================================
//  "Mixed" string copy.  Copies narrow string into wide buffer. Max of count Count
...
xwchar* x_mstristr( const xwchar* pMainStr, const char* pSubStr )
{
    ...
    while( *pM )
    {
        ...
        while( *pS1 && *pS2 && !(x_tolower((char)*pS1) - x_tolower(*pS2)) )
        {
            pS1++;
            pS2++;
        }
...
char* x_mstristr( const char* pMainStr, const xwchar* pSubStr )
{
    ...
    while( *pM )
    {
        ...
        while( *pS1 && *pS2 && !(x_tolower(*pS1) - x_tolower((char)*pS2)) )
        {
            pS1++;
            pS2++;
        }
...
char* x_strncpy( char* pDest, const char* pSrc, s32 Count )
{
    ...
    // The ANSI standard does not terminate a truncated string with \0
    // We do...
    //pDest[ Count-1 ] = 0;
...
s32 x_wstricmp( const xwchar* pStr1, const xwchar* pStr2 )
{
    s32 C1, C2;
...
static u32      Reflect(u32 ref, char ch);    // Reflects CRC bits in the lookup table 
static xbool    crc_initialized = FALSE;
...
//==============================================================================
// Encryption is Tiny Encryption Algorithm (TEA) based encryption. 
// This was obtained from http://www.simonshepherd.supanet.com/tea.htm and as
// noted on the web site is royalty free, license free and in the public domain.
//
// Original algorithm by Roger Needham and David Wheeler. This implementation
...
void encipher(u32 *const v, u32 *const w, const u32 *const k)
{
    u32 y=v[0],z=v[1];
    u32 sum=0;
    u32 delta=ENCRYPT_DELTA;
    u32 a=k[0],b=k[1],c=k[2],d=k[3];
    u32 n=ENCRYPT_ITERATIONS;
...
void x_decrypt( void* pData, s32 Length, u32 Key[4] )
{
    ...
    ASSERTS( (Length & 7) == 0, "Decrypted data block must be a multiple of 8 bytes in length" );

    while( Length > 0 )
    {
        decipher( pData32, pData32, Key );
        Length -= 8;
        pData32 += 2;
    }
}
...
#if defined( X_DEBUG )
        u32 Original[2];
        Original[0] = pData32[0];
        Original[1] = pData32[1];
#endif
        encipher( pData32, pData32, Key );
#if defined( X_DEBUG )
        // Verify that the encrypted data block matches a decrypted data block
        u32 Temp[2];
        decipher( pData32, Temp, Key );
        ASSERTS( (Original[0] == Temp[0]) && (Original[1] == Temp[1]), "Encryption verification failed." );
#endif
        pData32+= 2;
        Length -= 8;
    }
