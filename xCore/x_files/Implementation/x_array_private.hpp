...
T& xarray<T>::Insert( s32 Index )
{
    ...
    if( m_Capacity < m_Count+1 )
    {
        ...
        s32 NewCapacity = m_Count + CalcGrowth();
        ...
        // Copy the index of the inserted element.
        iElement = Index; 
...
const xarray<T>& xarray<T>::operator = ( const xarray<T>& Array )
{
    ...
    if( this != &Array )
    {
        ...
        for( i = 0; i < Array.m_Count; i++ )
        {
            ASSERTS(m_pData, "xarray::operator= attempting to write to NULL uninitialized array");
            m_pData[i] = Array.m_pData[i];
        }
...
template< class T >
xbool xarray<T>::Save( X_FILE* pFile ) const
{
    ASSERT( pFile );
...
template< class T >
xbool xarray<T>::Load( X_FILE* pFile )
{
    ASSERT( pFile );
