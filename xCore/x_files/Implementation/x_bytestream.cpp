...
{
    MEMORY_OWNER( "xbytestream::xbytestream()" );
...
void xbytestream::Replace( s32 Index, byte Data )
{
    ASSERT( Index >= 0 );
    ASSERT( (Index + 1) <= STREAM_LENGTH );
...
void xbytestream::Replace( s32 Index, const byte* pData, s32 Count )
{
    if( Count == 0 ) return;
...
void xbytestream::Replace( s32 Index, const xbytestream& Bytestream )
{
    ASSERT( Index >= 0 );
    ASSERT( this  != &Bytestream );     // Cannot insert an xbytestream into itself!
    ...
    // Insert the stream.
    x_memcpy( m_pData+Index, Bytestream.m_pData, Bytestream.GetLength() );
}
...
const xbytestream& xbytestream::operator << ( u8    Data )
{
    EnsureCapacity( STREAM_LENGTH + 1 );
    *(u8*)&m_pData[STREAM_LENGTH] = Data;
    STREAM_LENGTH += 1;
    return( *this );
}

const xbytestream& xbytestream::operator << ( s8    Data )
{
    EnsureCapacity( STREAM_LENGTH + 1 );
    *(s8*)&m_pData[STREAM_LENGTH] = Data;
    STREAM_LENGTH += 1;
    return( *this );
}

const xbytestream& xbytestream::operator << ( u16   Data )
{
    EnsureCapacity( STREAM_LENGTH + 2 );
    *(u16*)&m_pData[STREAM_LENGTH] = Data;
    STREAM_LENGTH += 2;
    return( *this );
}

const xbytestream& xbytestream::operator << ( s16   Data )
{
    EnsureCapacity( STREAM_LENGTH + 2 );
    *(s16*)&m_pData[STREAM_LENGTH] = Data;
    STREAM_LENGTH += 2;
    return( *this );
}

const xbytestream& xbytestream::operator << ( u32   Data )
{
    EnsureCapacity( STREAM_LENGTH + 4 );
    *(u32*)&m_pData[STREAM_LENGTH] = Data;
    STREAM_LENGTH += 4;
    return( *this );
}

const xbytestream& xbytestream::operator << ( s32   Data )
{
    EnsureCapacity( STREAM_LENGTH + 4 );
    *(s32*)&m_pData[STREAM_LENGTH] = Data;
    STREAM_LENGTH += 4;
    return( *this );
}
...
xbool xbytestream::LoadFile( X_FILE* pFile, s32 Size )
{
    s32     Read;
...
xbool xbytestream::SaveFile( X_FILE* pFile ) const
{
    s32     Written;
...
xbool xbytestream::LoadFile( const char* pFileName )
{
    ...
    xbool Success = LoadFile( pFile, Size );
