...
void audio_stream_read_callback( io_request* pRequest, audio_stream* pStream, s32 ReadBufferIndex )
{
    ...
    switch( pStream->Type )
    {
        case MONO_STREAM: 
        {
            s_ChannelManager.PushData( (void*)(g_AudioStreamMgr.m_ReadBuffers[ReadBufferIndex]),
                                       (void*)pStream->ARAM[LEFT_CHANNEL][pStream->ARAMWriteBuffer], 
        ...
        case STEREO_STREAM:
        {
            s_ChannelManager.PushData( (void*)(g_AudioStreamMgr.m_ReadBuffers[ReadBufferIndex]), 
                                       (void*)pStream->ARAM[LEFT_CHANNEL][pStream->ARAMWriteBuffer], 
                                       STREAM_BUFFER_SIZE, NULL );
            s_ChannelManager.PushData( (void*)(g_AudioStreamMgr.m_ReadBuffers[ReadBufferIndex]+STREAM_BUFFER_SIZE), 
                                       (void*)pStream->ARAM[RIGHT_CHANNEL][pStream->ARAMWriteBuffer], 
                                       STREAM_BUFFER_SIZE, NULL );
            break;
        }
...
void audio_stream_mgr::Init( void )
{
    ...
    MEMORY_OWNER( "audio_stream_mgr::Init()" );
    ...
    for( i=0 ; i<MAX_AUDIO_STREAMS ; i++ )
    {
        ...
        // Assign ARAM to each stream channel.
