...
//
// Description: This example debugger script shows how to use scripts to fetch memory from the target console.
//                                                                          
//****************************************************************************
//
//
...
//
// startaddr	The address to start dumping at.
// endaddr	The address to finish at.
...
void dumpmem(SNADDR startaddr, SNADDR endaddr, int size)
{
	...
	int		modulo = 16;	// Add 16 to get to the next line.
	SNADDR	temp;
	...
	else
	{
		if(endaddr - startaddr > 1024)	 // Sanity check the range.
			endaddr = startaddr + 1024; // Limit it to something that won't take too long.
