...
const char* GetPicInfo( const char* Path, s32& XCount, s32& YCount, s32& X, s32& Y )
{
    ...
    char* pInfo = x_stristr(FName, "_") ;
    if (!pInfo)
        return NULL ;

    // Search for info
    s32 Number = 0 ;
    x_strtolower( pInfo );  // Convert to lower case so scanf works!
    if (sscanf(pInfo, "_%dx%d_%d", &XCount, &YCount, &Number) != 3)
        return NULL ;

    // Get components of filename to reserve case again
    x_splitpath(Path, Drive, Dir, FName, Ext) ;
...
void main( s32 argc, char* argv[] )
{
    ...
    x_printf("                                                                              ") ;
    x_printf("Usage:  PicJoinTool.exe <Optional Search Path>                                ") ;
    x_printf("                                                                              ") ;
    ...
    // Setup search path from command line?
    char SearchPath[X_MAX_PATH] = {0} ;
    if (argc == 2)
	    x_strcpy(SearchPath, argv[1]) ;
    
    // Use default search?
    if (x_strlen(SearchPath) == 0)
	    x_strcpy(SearchPath, "*.tga") ;
    ...
    x_splitpath(SearchPath, Drive, Dir, NULL, NULL) ;

	// Find all tgas
	_finddata_t FileData;
	long        FindData ;
	FindData = _findfirst(SearchPath, &FileData) ;
	if(FindData != -1)
    {
		do
        {
			...
			if (FileData.name)
            {
                // Convert to full path filename
                x_splitpath(FileData.name, NULL, NULL, FName, Ext) ;
                x_makepath(FileData.name, Drive, Dir, FName, Ext) ;
                ...
                if (BigPicName)
                {
                    ...
                    {
                        // Matching names?
                        if (x_stricmp(BigPicList[i].Name, BigPicName) == 0)
                        {
