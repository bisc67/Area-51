...
static conditional_enum_pair s_ConditonalAllEnumTable[] = 
{
        ...
        conditional_enum_pair("OBJECT_HEALTH",       conditional_base::TYPE_CONDITION_OBJECT_HEALTH),
        conditional_enum_pair("BUTTON_PRESSED",      conditional_base::TYPE_CONDITION_ON_THIS_BUTTON),
        conditional_enum_pair("PLAYER_STRAIN",       conditional_base::TYPE_CONDITION_PLAYER_STRAIN),
...
{ 
    ...
    f32 CurrentHealth = PlayerObj.m_pObject->GetHealth();
...
//=========================================================================
// OBJECT_HEALTH : checks on objects health...
//=========================================================================

condition_create_function object_health::m_Register( TYPE_CONDITION_OBJECT_HEALTH, object_health::Create );

object_health::code_pair object_health::s_PairTable[] = 
{
        code_pair("GREATER_INCLUSIVE", object_health::CODE_GREATER_INCLUSIVE),
        code_pair("LESSER_INCLUSIVE",  object_health::CODE_LESSER_INCLUSIVE),
        code_pair("GREATER",           object_health::CODE_GREATER),
        code_pair("LESSER",            object_health::CODE_LESSER),
        code_pair("EQUAL",             object_health::CODE_EQUAL),
        code_pair("NOT_EQUAL",         object_health::CODE_NOT_EQUAL),
        
        code_pair( k_EnumEndStringConst,object_health::INVALID_CODES),  //**MUST BE LAST**//
};

object_health::code_table object_health::s_EnumTable( s_PairTable );

//=============================================================================

object_health::object_health(  guid ParentGuid ):  conditional_base(  ParentGuid ), m_Code(CODE_GREATER_INCLUSIVE), m_Value(0.0f), m_ObjectGuid(0)
{}
...
{ 
    ...
    object_ptr<object> pObj( m_ObjectGuid );

    if ( !pObj.IsValid( ) )
        return TRUE;
    
    f32 CurrentHealth = pObj.m_pObject->GetHealth();
...
void object_health::OnEnumProp ( prop_enum& rPropList )
{   
    //object info
...
xbool object_health::OnProperty ( prop_query& rPropQuery )
{
...
const char*  object_health::GetInfo( void )
{
    static big_string   Info;
    ...
    {
    case CODE_GREATER_INCLUSIVE:   
        Info.Set(xfs("%s If the objects Health is Greater or Equal to %3.3f.", LogicFlag.Get(), m_Value));      break;
    case CODE_LESSER_INCLUSIVE:     
        Info.Set(xfs("%s If the objects Health is Less than or Equal to %3.3f.", LogicFlag.Get(), m_Value));    break;
    case CODE_GREATER:              
        Info.Set(xfs("%s If the objects Health is Greater than %3.3f.", LogicFlag.Get(), m_Value));             break;
    case CODE_LESSER:               
        Info.Set(xfs("%s If the objects Health is Les than %3.3f.", LogicFlag.Get(), m_Value));                 break;
    case CODE_EQUAL:                
        Info.Set(xfs("%s If the objects Health is Equal to %3.3f.", LogicFlag.Get(), m_Value));                 break;
    case CODE_NOT_EQUAL:            
        Info.Set(xfs("%s If the objects Health is Not Equal to %3.3f.", LogicFlag.Get(), m_Value));             break;
    default:
        ASSERT(0);
        break;
    }
...
player_has::player_has(  guid ParentGuid ):  conditional_base(  ParentGuid ), m_Code(CODE_HAS_ITEM)
{
    m_ItemType = inventory_item::INV_ANTI_MUTAGEN;
}
...
{  
    ...
    xbool bHasItem = pPlayer->GetInventory().GetCollapsedListPtr( m_ItemType )->m_ItemCount > 0;
    xbool bHasWeapon = pPlayer->GetWeaponInventory().GetCollapsedListPtr( m_ItemType )->m_ItemCount > 0;
...
{ 
    ...
    rPropList.AddEnum (  "Item Type", inventory::GetItemEnum(),
                         "Item to check." ) ;
...
xbool player_has::OnProperty ( prop_query& rPropQuery )
{
    ...
    if( rPropQuery.IsVar( "Item Type" ) )
    {
        if ( rPropQuery.IsRead() )
        {
            rPropQuery.SetVarEnum( inventory::GetItemName( m_ItemType ) );
        }
        else
        {
            m_ItemType = inventory::GetItemByName( rPropQuery.GetVarEnum() );
        }
        return TRUE;
    }
...
//        return TRUE;
    ...
    if ( rPropQuery.IsVar( "Logic") )
    {
        ...
        else
        {
            ...
            if( x_stricmp( pString, "HAS_ITEM" )==0) {          m_Code = CODE_HAS_ITEM;}
...
const char*  player_has::GetInfo( void )
{
    ...
    Info.Set(xfs("%s If the Player Has an item %s.", LogicFlag.Get(), inventory::GetItemName(m_ItemType) ) );
...
condition_create_function player_strain::m_Register( TYPE_CONDITION_PLAYER_STRAIN, player_strain::Create );

//=============================================================================

player_strain::player_strain(  guid ParentGuid ):  conditional_base(  ParentGuid ), m_Code(CODE_IS_STRAIN)
{
    m_StrainToCheck = player::STRAIN_HUMAN;
}
...
{  
    ...
    xbool bValidPlayer = pPlayer->IsKindOf( player::GetRTTI() );
    ...
    xbool bIsStrain = pPlayer->GetCurrentStrain() == m_StrainToCheck;;
...
void player_strain::OnEnumProp ( prop_enum& rPropList )
{ 
    //object info
    rPropList.AddInt ( "Code" , "",  PROP_TYPE_DONT_SHOW  );
    
    rPropList.AddEnum (  "Strain Type", player::GetStrainEnum(),
                         "Strain to check." ) ;
    
    rPropList.AddEnum( "Logic", 
        "IS_STRAIN\0IS_NOT_STRAIN\0", "Logic available to this condtional.", PROP_TYPE_DONT_SAVE  );
...
xbool player_strain::OnProperty ( prop_query& rPropQuery )
{
    if ( rPropQuery.VarInt ( "Code" , m_Code ) )
        return TRUE;
  
    if( rPropQuery.IsVar( "Strain Type" ) )
    {
        if ( rPropQuery.IsRead() )
        {
            rPropQuery.SetVarEnum( player::GetStrainName( m_StrainToCheck ) );
        }
        else
        {
            m_StrainToCheck = player::GetStrainByName( rPropQuery.GetVarEnum() );
        }
        return TRUE;
    }
    ...
    if ( rPropQuery.IsVar( "Logic") )
    {
        if( rPropQuery.IsRead() )
        {
            switch ( m_Code )
            {
            case CODE_IS_STRAIN:             rPropQuery.SetVarEnum( "IS_STRAIN" );            break;
            case CODE_IS_NOT_STRAIN:         rPropQuery.SetVarEnum( "IS_NOT_STRAIN" );        break;
            default:
                ASSERT(0);
                break;
            }
        ...
        else
        {
            ...
            if( x_stricmp( pString, "IS_STRAIN" )==0) {          m_Code = CODE_IS_STRAIN;}
            if( x_stricmp( pString, "IS_NOT_STRAIN" )==0) {      m_Code = CODE_IS_NOT_STRAIN;}
...
const char*  player_strain::GetInfo( void )
{
    static big_string   Info;
    
    sml_string  LogicFlag ( GetFlagStr(m_Flag) );

    Info.Set(xfs("%s If the player is a strain %s.", LogicFlag.Get(), player::GetStrainName( m_StrainToCheck ) ) );
...
//=========================================================================
// ON_THIS_BUTTON  : check if a button has been pressed by the player.  Only for
//                   use on spatial triggers.
//=========================================================================

condition_create_function on_this_button::m_Register( TYPE_CONDITION_ON_THIS_BUTTON, on_this_button::Create );

//=============================================================================

on_this_button::on_this_button(  guid ParentGuid ):  conditional_base(  ParentGuid )
{
...
{  
    ...
    {
        x_try;
        x_throw( "WARNING!: There is an invalid ON_THIS_BUTTON condition that was just tested.  All ON_THIS_BUTTON conditions must be on spatial triggers only" );
        x_catch_display;

        return FALSE;
    }

    // We need the trigger to update all the time.  Also, we need to set the response type to 
    // 'repeating'.
    trigger_spatial_object* pSpatial = ( trigger_spatial_object* ) pParent;
    pSpatial->SetResponseType( trigger_spatial_object::RESPONSE_IF_STILL_INSIDE );
    pSpatial->SetUpdateRate( 0.01f );
    player* pPlayer = SMP_UTIL_GetActivePlayer();
    ...
    xbool WasPressed = g_IngamePad[ pPlayer->GetActivePlayerPad() ].GetLogical( m_ButtonID ).WasValue > 0.0f;
    xbool IsPressed = g_IngamePad[ pPlayer->GetActivePlayerPad() ].GetLogical( m_ButtonID ).IsValue > 0.1f;
...
void on_this_button::OnEnumProp ( prop_enum& rPropList )
{ 
    rPropList.AddEnum (  "Logical Input", ingame_pad::GetLogicalIDEnum(),
                         "Logical mapping to watch." ) ;
...
xbool on_this_button::OnProperty ( prop_query& rPropQuery )
{  
...
const char*  on_this_button::GetInfo( void )
{
    static big_string   Info;
    
    Info.Set(xfs("If the active controller has had button %s pressed.", ingame_pad::GetLogicalIDName(m_ButtonID) ) );
