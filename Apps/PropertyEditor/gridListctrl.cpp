...
m_BackgroundColor(RGB(210,210,210))
{
...
void CGridListCtrl::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct) 
{
	...
	if(lpDrawItemStruct->itemAction == ODA_DRAWENTIRE)
	{
		...
		COLORREF crBorder = RGB(MAX(GetRValue(m_BackgroundColor) - 30, 0),
                                MAX(GetGValue(m_BackgroundColor) - 30, 0),
                                MAX(GetBValue(m_BackgroundColor) - 30, 0));
		...
		else
		{
			if (lpDrawItemStruct->itemState & ODS_SELECTED)
			{
			    ...
			    crBackground = RGB(MAX(GetRValue(m_BackgroundColor) - 120, 0),
                                   MAX(GetGValue(m_BackgroundColor) - 120, 0),
                                   MAX(GetBValue(m_BackgroundColor) - 75,  0));
				if (bReadOnlyItem)
				{
			...
			else
			{
                ...
                if( bHeader )
                {
                    crBackground = m_BackgroundColor;
                    crText       = RGB(64,64,64);
                }
                else
                {
				    if (nItem%2 == 0)
				    {
			            crBackground = RGB(MIN(GetRValue(m_BackgroundColor) + 45, 255),
                                           MIN(GetGValue(m_BackgroundColor) + 45, 255), 
                                           MIN(GetBValue(m_BackgroundColor) + 45, 255));
				    }
				    else
				    {
					    //crBackground = RGB(235,235,255);
					    crBackground = RGB(MIN(GetRValue(m_BackgroundColor) + 30, 255),
                                           MIN(GetGValue(m_BackgroundColor) + 30, 255), 
                                           MIN(GetBValue(m_BackgroundColor) + 30, 255));
				    }
	...
	if(pInfo->GetControlType(nColumn-1, ctrlType))
	{
		switch(ctrlType)
		{
			...
			case pInfo->CONTROLTYPE::GCT_DIR_BUTTON:
			{
                ...
                if (ctrlType == pInfo->CONTROLTYPE::GCT_GUID_EDIT)
				    pDC->DrawText("G",rect,DT_CENTER);
                else
...
BOOL CGridListCtrl::HitTestOnSplitter( CPoint point )
{
    return x_abs( point.x - GetColumnWidth( 0 ) ) <= 2;
}
...
void CGridListCtrl::OnMouseMove(UINT nFlags, CPoint point) 
{
    if( m_bSplitterDrag )
    {
        ...
        int Width = m_SplitterDragStartWidth + Delta;
        if( Width < 6 ) Width = m_SplitterDragStartWidth;
        if( Width > ( r.Width() - 6 ) ) Width = m_SplitterDragStartWidth;
        int nWidth = r.Width() - Width;

        // Set column widths so the combined total never exceeds the window width,
        // that stops the scroll bar popping onto the screen
        if( Width < GetColumnWidth(0) )
        {
