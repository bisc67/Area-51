        ...
        if (iCount > ht.iItem)
        {
		    ...
		    if(pItem)
		    {
                ...
                if (lp)
                {
                    ...
                    m_pToolTip->SetWindowPos( &wndTop, 0, 0, 0, 0, SWP_NOMOVE|SWP_NOSIZE|SWP_SHOWWINDOW|SWP_NOACTIVATE );
...
/*
//    CGridTreeItem* pFound = FindChildWithIdentifier( lpInfo->GetItemText(), pParent );
//    if( pFound )
//        return NULL;
*/
			...
			case CGridItemInfo::GCT_3D_COORDINATE:
				{
                    ...
                    COLORREF ItemColor           = pItem->m_BackgroundColor;
...
void CPropertyGrid::OnTreeItemSelected(CGridTreeItem* pItem)
{
    if (pItem)
    {
        ...
        CWnd* pTop = ::AfxGetMainWnd();

        if (!GetParent()->SendMessage(WM_USER_MSG_GRID_SELECTION_CHANGE,(long)pItem,0))
	    {
...
void CPropertyGrid::OnGridItemChange(CGridTreeItem *pSelItem) 
{
    if (pSelItem)
    {
        ...
        else if (!GetParent()->SendMessage(WM_USER_MSG_GRID_ITEM_CHANGE,(long)pSelItem,0))
	    {
